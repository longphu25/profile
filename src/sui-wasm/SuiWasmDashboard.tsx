...
const PLUGIN_GROUPS: PluginGroup[] = [
  {
    ...
    plugins: [
      ...
      },
      {
        ...
        name: 'SuiZkLogin',
        label: 'ZK Login',
        desc: 'OAuth → ZK Proof → Wallet (devnet)',
        src: pluginPath('sui-zk-login'),
        wasmDesc: 'zkLogin + Ed25519 + Mysten prover',
      },
      {
        id: 'sui-zk-merkle',
        name: 'SuiZkMerkle',
        label: 'ZK Merkle',
        desc: 'Build Merkle tree → identity.json blobs',
        src: pluginPath('sui-zk-merkle'),
        wasmDesc: 'Rust WASM — SHA-256 Merkle tree (72KB)',
      },
    ],
  },
  {
    ...
    plugins: [
      ...
      },
      {
        ...
        label: 'NAVI Protocol',
        desc: 'Lending, swap, bridge, vaults via MCP',
        src: pluginPath('sui-navi-dashboard'),
        wasmDesc: 'NAVI MCP (no auth, read-only)',
      },
      {
        id: 'sui-navi-advisor',
        name: 'SuiNaviAdvisor',
        label: 'NAVI Advisor',
        desc: 'Yield strategy recommendations',
        src: pluginPath('sui-navi-advisor'),
        wasmDesc: 'NAVI MCP pools + Volo vaults',
      },
      {
        id: 'sui-navi-chatbot',
        name: 'SuiNaviChatbot',
        label: 'NAVI Chatbot',
        desc: 'Chat-based DeFi advisor via MCP',
        src: pluginPath('sui-navi-chatbot'),
        wasmDesc: 'NAVI MCP 37 tools — conversational',
      },
      {
        id: 'sui-navi-analysis',
        name: 'SuiNaviAnalysis',
        label: 'NAVI Analysis',
        desc: 'Real-time pool stats & best yield finder',
        src: pluginPath('sui-navi-analysis'),
        wasmDesc: 'WASM-ready analysis engine — 15s auto-refresh',
      },
    ],
  },
  {
    ...
    plugins: [
      {
        ...
        name: 'SuiSealPrivate',
        label: 'Private Data',
        desc: 'Encrypt data only you can decrypt',
        src: pluginPath('sui-seal-private'),
        wasmDesc: '@mysten/seal + private_seal pattern',
      },
      {
        id: 'sui-seal-timelock',
        name: 'SuiSealTimelock',
        label: 'Time-Lock',
        desc: 'Data unlocks after a deadline',
        src: pluginPath('sui-seal-timelock'),
        wasmDesc: '@mysten/seal + timelock_seal + Clock',
      },
      {
        id: 'sui-seal-allowlist',
        name: 'SuiSealAllowlist',
        label: 'Allowlist',
        desc: 'Group access — only listed addresses decrypt',
        src: pluginPath('sui-seal-allowlist'),
        wasmDesc: '@mysten/seal + allowlist_seal on-chain',
      },
      {
        id: 'sui-seal-encrypt',
        name: 'SuiSealEncrypt',
        label: 'Encrypt',
        desc: 'Threshold-encrypt data with Seal',
        src: pluginPath('sui-seal-encrypt'),
        wasmDesc: '@mysten/seal + @noble/curves IBE',
      },
      {
        id: 'sui-seal-decrypt',
        name: 'SuiSealDecrypt',
        label: 'Decrypt',
        desc: 'Decrypt with SessionKey + wallet approval',
        src: pluginPath('sui-seal-decrypt'),
        wasmDesc: '@mysten/seal + SessionKey signing',
      },
      {
        id: 'sui-seal-vault',
        name: 'SuiSealVault',
        label: 'Secret Vault',
        desc: 'Encrypted secrets manager',
        src: pluginPath('sui-seal-vault'),
        wasmDesc: '@mysten/seal + localStorage',
      },
      {
        id: 'sui-seal-walrus',
        name: 'SuiSealWalrus',
        label: 'Seal + Walrus',
        desc: 'Encrypted file storage on Walrus',
        src: pluginPath('sui-seal-walrus'),
        wasmDesc: '@mysten/seal + Walrus HTTP API',
      },
      {
        id: 'sui-seal-voting',
        name: 'SuiSealVoting',
        label: 'Sealed Voting',
        desc: 'Encrypted ballots — votes hidden until tally',
        src: pluginPath('sui-seal-voting'),
        wasmDesc: '@mysten/seal + HMAC-CTR + on-chain decrypt',
      },
      {
        id: 'sui-seal-walrus-upload',
        name: 'SuiSealWalrusUpload',
        label: 'Seal Upload',
        desc: 'Encrypt file → upload to Walrus',
        src: pluginPath('sui-seal-walrus-upload'),
        wasmDesc: '@mysten/seal encrypt + Walrus publisher',
      },
    ],
  },
]
