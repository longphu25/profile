...
/**
 * Surface Studio orchestrator. Loads the same predict-club plugin and mounts the
 * React-owned `PredictClub.Surface.Studio` into the `#pc-studio-root` slot. Wallet
 * wiring mirrors `src/predict-club-next/main.tsx`; the data layer is shared, so this
 * surface never forks context or domain logic.
 */
export function PredictSurfaceStudioOrchestrator() {
  ...
  // Load plugin and mount the Studio shell into its root slot
  useEffect(() => {
    ...
    async function loadAndMount() {
      try {
        ...
        const container = document.getElementById('pc-studio-root')
        const Shell = suiHostAPI.getComponent('PredictClub.Surface.Studio') as
  ...
  if (error) {
    console.error('[PredictSurfaceStudio] Mount error:', error)
  }
...
createRoot(rootEl).render(
  <StrictMode>
    <DAppKitProvider dAppKit={dAppKit as unknown as DefaultExpectedDppKit}>
      <PredictSurfaceStudioOrchestrator />
    </DAppKitProvider>
  </StrictMode>,
)
