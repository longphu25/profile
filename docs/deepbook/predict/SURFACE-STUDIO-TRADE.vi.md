...
## Hiển thị cell: payout multiple làm số chính

Trader binary quét grid để tìm "đặt 1 ăn mấy", không phải để đọc implied vol. Nên một
cell trong ATM band của cột đang chọn, nơi có quote contract thật, hiện payout multiple
làm số chính: `payoutMultiple = 1 / contractProbability` (`domain/volSurface.ts`). Binary
trả 1 DUSDC mỗi contract khi thắng, nên nghịch đảo xác suất thắng hợp đồng ngụ ý chính là
multiple thật đã gồm vig mà trader nhận (hợp đồng xác suất 0.62 trả 1.6x). IV lùi xuống
dòng phụ nhỏ và nền vẫn là thang màu IV, nên mặt vol-surface vẫn đọc được như cũ.

Cell ngoài band không có quote nên giữ IV làm số chính thay vì multiple suy từ model:
multiple từ model sẽ quảng cáo payout đẹp hơn hợp đồng thật trả (vì bỏ qua biên nhà cái).
Multiple do đó chỉ hiện nơi có quote thật chống lưng, không bịa để lấp grid.
`formatMultiple` cap multiple sâu ngoài tiền ở "9x+" để không tràn cell, và tooltip thêm
dòng Payout bên cạnh xác suất model và edge.
