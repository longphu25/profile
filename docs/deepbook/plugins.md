...
| 9 | `sui-deepbook-hedging-bot` | ✅ Done | v0.21.0 |
...
### 9. `sui-deepbook-hedging-bot` — Client-Side Hedging Bot

**Priority:** 🥇 | **Effort:** High | **Status:** ✅ Done

Browser-based hedging bot — no server needed. Full documentation: [hedging-bot.md](hedging-bot.md)

- Generate 2 Ed25519 wallets or import from sui.keystore
- Encrypted vault export (.vault) — AES-256-GCM for iPad/cross-device
- Auto-balance: split SUI + swap base token via DeepBook SDK
- Bot cycle loop: open → hold → close with configurable timing
- 2-column layout: mini orderbook + funding check + points estimator
- Live volatility ranking from `/summary` API
- All coin balances display via `suix_getAllBalances`
- Points estimation based on maker volume

**Data source:** DeepBook Indexer + `@mysten/deepbook-v3` SDK + `@mysten/sui` keypairs
**Dedicated page:** `sui-deepbook-hedging-bot.html`
