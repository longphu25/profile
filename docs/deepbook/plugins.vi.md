...
### 9. `sui-deepbook-hedging-bot` — Hedging Bot Chạy Phía Trình Duyệt

**Ưu tiên:** 🥇 | **Công sức:** Cao | **Trạng thái:** ✅ Done

Hedging bot chạy trong browser, không cần server. Tài liệu đầy đủ:
[hedging-bot.md](hedging-bot.md)

- Sinh 2 ví Ed25519 hoặc import từ `sui.keystore`
- Xuất encrypted vault (`.vault`) — AES-256-GCM cho iPad/đa thiết bị
- Auto-balance: chia SUI + swap base token qua DeepBook SDK
- Vòng lặp bot: open → hold → close với thời gian cấu hình được
- Layout 2 cột: mini orderbook + funding check + points estimator
- Xếp hạng biến động live từ API `/summary`
- Hiển thị toàn bộ coin balance qua `suix_getAllBalances`
- Ước tính points dựa trên maker volume

**Nguồn dữ liệu:** DeepBook Indexer + SDK `@mysten/deepbook-v3` + keypair `@mysten/sui`
**Trang riêng:** `sui-deepbook-hedging-bot.html`
