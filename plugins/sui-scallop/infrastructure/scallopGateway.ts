...
async function getScallop(walletAddress?: string): Promise<Scallop> {
  if (!scallopInstance) {
    scallopInstance = new Scallop({ networkType: 'testnet', walletAddress } as any)
  ...
  return {
    ...
    async buildBorrowUsdcTx(params) {
      ...
      if (params.obligationId) {
        await txBlock.depositCollateralQuick(params.collateralSuiAmount, 'sui', params.obligationId)
        await txBlock.borrowQuick(params.borrowUsdcAmount, 'usdc', params.obligationId)
      } else {
        const [obligation, obligationKey, hotPotato] = txBlock.openObligation()
        await txBlock.depositCollateralQuick(params.collateralSuiAmount, 'sui', obligation)
        await txBlock.borrowQuick(params.borrowUsdcAmount, 'usdc', obligation, obligationKey)
