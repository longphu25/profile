...
export function DashboardTab(props: DashboardTabProps) {
  const {
    running, stage, config, cycleNum, currentPrice, totalPnl, totalVolume,
    holdStart, holdEnd, addrA, addrB, network,
    history, logs, setLogs,
  } = props

  const holdProgress = holdStart && holdEnd
    ...
    <>
      ...
      <div className="sui-hb__stats">
        {[
          ...
          { label: 'PnL', value: formatUsd(totalPnl), color: totalPnl >= 0 ? '#22c55e' : '#ef4444' },
          { label: 'Volume', value: formatUsd(totalVolume) },
          { label: 'Est. Points', value: `~${Math.round(totalVolume)}` },
          { label: 'Cycles', value: `${cycleNum}${config.maxCycles ? `/${config.maxCycles}` : ''}` },
          { label: 'Pool', value: config.pool },
        ].map((s) => (
          <div key={s.label} className="sui-hb__stat">
            <span className="sui-hb__stat-label">{s.label}</span>
            <span className="sui-hb__stat-value" style={s.color ? { color: s.color } : undefined}>{s.value}</span>
          </div>
        ))}
      </div>
        ...
        <div className="sui-hb__card" style={{ marginTop: 8 }}>
          {addrA && (
            <div style={{ fontSize: 10, color: '#94a3b8', marginBottom: 4 }}>
              A: <a href={`https://suiscan.xyz/${network}/account/${addrA}`} target="_blank" rel="noopener noreferrer" style={{ color: '#4da2ff', textDecoration: 'none', fontFamily: "'Fira Code', monospace" }}>{addrA.slice(0, 10)}…{addrA.slice(-4)}</a>
            </div>
          )}
          {addrB && (
            <div style={{ fontSize: 10, color: '#94a3b8' }}>
              B: <a href={`https://suiscan.xyz/${network}/account/${addrB}`} target="_blank" rel="noopener noreferrer" style={{ color: '#a78bfa', textDecoration: 'none', fontFamily: "'Fira Code', monospace" }}>{addrB.slice(0, 10)}…{addrB.slice(-4)}</a>
            </div>
          )}
        </div>
      )}
      ...
      {history.length > 0 && (
        <div className="sui-hb__card" style={{ marginTop: 8 }}>
          ...
          <div style={{ maxHeight: 150, overflow: 'auto' }}>
            {history.map((h) => (
              <div key={h.num} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, padding: '3px 0', borderBottom: '1px solid #1e293b' }}>
                <span style={{ color: '#64748b' }}>#{h.num}</span>
                <span style={{ color: '#94a3b8', fontFamily: "'Fira Code', monospace" }}>{formatOBPrice(h.openPrice)} → {formatOBPrice(h.closePrice)}</span>
                <span style={{ color: h.pnl >= 0 ? '#22c55e' : '#ef4444' }}>{formatUsd(h.pnl)}</span>
      ...
      <div className="sui-hb__card" style={{ marginTop: 8 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button className="sui-hb__btn sui-hb__btn--ghost sui-hb__btn--sm" onClick={() => setLogs(() => [])}>Clear</button>
        </div>
