...
/**
 * Vol-surface heatmap (plan 23, S1): a strike x expiry implied-vol matrix.
 *
 ...
 *
 * The component owns the surface service lifecycle: it starts the SVI fan-out on
 * mount and stops it on unmount, so cockpit-only users never pay for it.
 */
...
function HeatmapBody({ grid }: { grid: SurfaceGrid }) {
  const { strikes, columns, ivRange } = grid
  // Grid template: a strike label column + one column per expiry.
  ...
  return (
    ...
    >
      {/* Header row: corner + expiry headers. */}
      <div
        role="columnheader"
      ...
      {columns.map((column) => (
        <div
          key={column.oracleId}
          role="columnheader"
          className="flex flex-col items-center justify-center bg-surface-container px-xs py-1 font-data text-data-sm text-on-surface"
        >
      ...
      {/* Strike rows. */}
      {strikes.map((strike) => (
        <Row key={strike} strike={strike} columns={columns} ivRange={ivRange} />
      ))}
    </div>
  )
}
...
export function VolHeatmap({ className = '' }: { className?: string }) {
  const [snap, setSnap] = useState<VolSurfaceSnapshot>(getVolSurfaceSnapshot)

  useEffect(() => {
    const unsub = subscribeVolSurface(setSnap)
    startVolSurfaceService()
    return () => {
      unsub()
      stopVolSurfaceService()
